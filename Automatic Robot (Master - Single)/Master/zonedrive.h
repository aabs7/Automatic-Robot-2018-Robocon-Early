...
const struct coordinates Throwingzone1 = {4700,1980};	//4700 -x 1900 -y
...
void updateZoneflag(void){
	...
	if(!ShuttleCockGiven || !GoldenRackGiven){
		...
		if(!ManualInFrontOfLZ2 && !((RACK_STATUSPORT & (1<<RACK_STATUSPIN))) && !(PINL & (1<<PL4)) && rotateGenevaOnce){
			uart3_putc('k');
			rotateGenevaOnce = false;
		}
