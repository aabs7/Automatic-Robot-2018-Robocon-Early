...
void updateZoneflag(void){
	...
	if(!(PINL & (1<<PL4)) &&  where == inLZ1 ){
		...
		GoThrowingZone1 = true;
		task3 = task4 = false;
		/*here we need 3 second delay just to hold the suttlecock*/
	}
	/*if low on shuttlecock pin then shuttlecock received
	i.e if manual robot arm is extended to give shuttlecock*/
	...
	if((!(SHUTTLECOCK_STATUSPORT & (1<<SHUTTLECOCK_STATUSPIN))) && (where == inLZ2 ) && !ManualInFrontOfLZ2){
	...
	if(ShuttleCockGiven && (PINL & (1<<PL4)) &&  where == inLZ1 ){
